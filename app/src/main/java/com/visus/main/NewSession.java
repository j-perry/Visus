...
public class NewSession extends Activity {
    ...
    private RectangleDrawableView rectDrawView;
    private TextDrawable textDraw;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LinearLayout status = (LinearLayout) findViewById(R.id.new_session_lyr_status);
        status.getLayoutParams().height = (h / 100) * 75;
        status.getLayoutParams().width = w;

        LinearLayout textStatus = (LinearLayout) findViewById(R.id.new_session_lyr_timer);
        textStatus.getLayoutParams().height = h / 2;
        textStatus.getLayoutParams().width = w / 3;

        /*
        statusParams.width = w;
        statusParams.height = h / 8;
        */

        /*
        ImageView im = (ImageView) this.findViewById(R.id.time);

        Bitmap bitmap = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(bitmap);
        im.setImageBitmap(bitmap);

        textDraw = new TextDrawable(canvas, "Hello", w / 3, h / 2);
        textDraw.display();

        rectDrawView = new RectangleDrawableView(this, w, h);
        */

        //View textVw = (View) findViewById(R.id.new_session_view);
        //textVw.bringToFront();

        //setContentView(rectDrawView);
    }
